---
title: "Investigate Potential Public Pension Plan Prototypes"
author: "Don Boyd"
...
outfn <- paste0("./Results/Prototypes_", format(Sys.time(), "%Y-%m-%d"), ".html")
...
df <- ppd %>% group_by(ppd_id, PlanName) %>%
  mutate(activepch=actives_tot / actives_tot[match(fy-1, fy)] * 100 - 100,
  ...
  summarise_each(funs(median(., na.rm=TRUE)), ActFundedRatio_GASB, InvestmentReturnAssumption_GASB,
                 PercentReqContPaid, NormCostRate_tot, activepch, retireepch, xcfpct, xcfpct2, abratio,
...
#df.prescale1 <- df %>% select(ppd_id, PlanName, ActFundedRatio_GASB, InvestmentReturnAssumption_GASB, PercentReqContPaid,
#                     NormCostRate_tot, activepch, xcfpct2, abratio, ActiveSalary_avg) 

cvars <- c("activepch", "retireepch", "xcfpct2", "abratio", "ActiveAge_avg") # we need the cluster vars names available later
df.prescale1 <- df %>% select(ppd_id, PlanName, one_of(cvars))

df.prescale2 <- na.omit(df.prescale1) # listwise deletion of missing (NYCPolice, NYCFire)

df.scaled <- df.prescale1 # 1= do NOT omit missing values, 2=DO omit missing values
...
df.clust <- cbind(group, df.prescale1) %>% # prescale1 or prescale2
  left_join(ppd %>% filter(fy==2013) %>% 
              select(ppd_id, MktAssets_net, PlanType, AdministeringGovt, ActFundedRatio_GASB, PercentReqContPaid)) %>%
...
df.dist <- df.all %>% group_by(group) %>%
  arrange(ppd_id) %>%
  do(getdist(., cvars)) # cvars was defined earlier
